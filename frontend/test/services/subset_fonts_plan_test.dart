...
void main() {
  test('Property 12: pyftsubset command construction', () {
    final List<FontSubsetStepPlan> steps =
    ...
    expect(
      ...
      contains(
        '--unicodes-file=C:/work\\subset_work\\${plan.randomName}\\Example.unicodes.txt',
      ),
    );
    expect(
      plan.pyftsubsetArguments,
      contains(
        '--output-file=C:/work\\subset_work\\${plan.randomName}\\Example.subset_tmp_.ttf',
      ),
    );
    ...
    expect(
      plan.pyftsubsetArguments,
      contains('--layout-features=vert,vrtr,vrt2,vkna'),
    );
    ...
    expect(plan.ttxDumpArguments, <String>[
      '-f',
      '-o',
      'C:/work\\subset_work\\${plan.randomName}\\Example.ttx',
      'C:/work\\subset_work\\${plan.randomName}\\Example.subset_tmp_.ttf',
    ]);
    ...
    expect(p.dirname(plan.outputFont.path), 'C:/work\\subsetted');
    expect(
      p.dirname(plan.codepointsFilePath),
      isNot(p.dirname(plan.outputFont.path)),
    );
    expect(
      p.dirname(plan.subsetTempPath),
      isNot(p.dirname(plan.outputFont.path)),
    );
    expect(p.dirname(plan.ttxXmlPath), isNot(p.dirname(plan.outputFont.path)));
  ...
  test('fonttools compatibility flags follow version thresholds', () {
    ...
    expect(
      fontToolsCompatibilityFlags('4.44.1'),
      contains('--no-prune-codepage-ranges'),
    );
    expect(
      fontToolsCompatibilityFlags('4.60.1'),
      contains('--drop-tables+=BASE'),
    );
    expect(
      fontToolsCompatibilityFlags(null),
      containsAll(<String>{
  ...
  test('Property 14: empty codepoint set still builds a full subset plan', () {
    ...
    expect(
      steps.single.pyftsubsetArguments,
      contains(
        '--unicodes-file=C:/work\\subset_work\\${steps.single.randomName}\\Empty.unicodes.txt',
      ),
    );
    expect(
      steps.single.outputFont.path,
      'C:/work\\subsetted\\Empty.subset.otf',
    );
  });

  test(
    'Property 22: license sidecar inputs and fsType warning are preserved',
    () {
      final List<FontSubsetStepPlan> steps =
          const FontAssetService(
            ffmpegPath: null,
